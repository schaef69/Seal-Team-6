 ...
 *
 *  Features:
 *   - Up to 8 players, each starts with $100
 *   - Bet types: number (35:1), color (1:1),
 *                odd/even (1:1), low/high (1:1)
 *   - Multiple bets per player per spin
 *   - Animated spinning wheel
 *   - Game ends when all players are broke
 */
...
typedef struct {
    ...
    int     value;    /* BET_COLOR:    0=red, 1=black          */
                      /* BET_ODD_EVEN: 1=odd, 0=even           */
                      /* BET_HIGH_LOW: 1=high(19-36), 0=low    */
    char    label[24];
} Bet;
...
static Color num_color(int n) {
    ...
    for (int i = 0; i < 18; i++)
        if (RED_NUMS[i] == n) return COL_RED;
    return COL_BLACK;
}

static void print_cell(int n) {
    Color c = num_color(n);
    const char *bg = (c == COL_RED)   ? BG_RED
                   : (c == COL_GREEN) ? BG_GRN
                                      : BG_BLK;
...
static int spin_wheel(void) {
    ...
    static const int DELAYS[] = {
        38, 40, 44, 49, 55, 62, 71, 82, 95, 110,
        128, 148, 170, 195, 222, 252, 285, 320
    };
    ...
    for (int f = 0; f < frames; f++) {
        int pos  = (landing - (frames - f) % 38 + 38 * 4) % 38;
        ...
        int delay = (f < nd) ? DELAYS[f] : DELAYS[nd-1] + (f - nd + 1) * 28;
...
/* ─── Evaluate one bet ───────────────────────────────────────────────────── */
/* Returns net gain (positive) or loss (negative, equals -stake) */
static double eval_bet(const Bet *b, int result) {
    ...
    case BET_COLOR:
        ...
        return ((b->value == 0 && rc == COL_RED) ||
                (b->value == 1 && rc == COL_BLACK))
               ? b->amount : -b->amount;
    case BET_ODD_EVEN:
        if (result == 0 || result == 37) return -b->amount;
        return ((b->value == 1 && result % 2 == 1) ||
                (b->value == 0 && result % 2 == 0))
               ? b->amount : -b->amount;
    case BET_HIGH_LOW:
        if (result < 1 || result > 36) return -b->amount;
        return ((b->value == 1 && result >= 19) ||
                (b->value == 0 && result <= 18))
               ? b->amount : -b->amount;
    }
    return -b->amount;
...
static void collect_bets(Player *p) {
    ...
    for (;;) {
        ...
        if (choice == 1) {
            ...
            fflush(stdout);
            scanf("%7s", buf); flush_stdin();
            if (strcmp(buf, "00") == 0) {
                b.number = 37;
                snprintf(b.label, sizeof(b.label), "00");
            } else {
                /* ensure all digits */
                for (int i = 0; buf[i]; i++)
        ...
        p->balance -= amount;          /* deduct stake immediately */
        p->bets[p->nbet++] = b;
        printf("  " GREEN "✓ Bet placed: $%.2f on %-12s" RST
               "  (Remaining: $%.2f)\n",
